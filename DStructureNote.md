...
作为树形结构之一，二叉树也和其他的树一样，树有什么特点呢？

- 有且仅有一个特定的称为”根(Boot)“的节点
- 当结点个数（我们用非负整数n去表示）>1时，其余的节点可以分为m个互不相交的有限集，每一个集合本身也是树，称为根的子树。
- 结点拥有的子树的个数称为结点的**度**，度为0的结点称为叶子或终端节点，树的度是树内各节点的度的最大值
- 结点的子树的根称为该结点的孩子，该节点邪恶称为孩子的双亲（比如D为A的子树的根，D就是A的孩子，A就是D的双亲）。同一个双亲的孩子之间互称兄弟。
- 结点的层次从根开始定义起 ，根为第一层，根的孩子为第二层，以此类推。

想一想树长什么样子，上面的这些概念就很好懂了。

下面我们给出二叉树的特性：

二叉树的特点是**每个结点至多只有两颗子树，并且子树有左右之分，顺序是不能颠倒的**。

| 概念       | 定义                                              | 关键特征                                       |
| ---------- | ------------------------------------------------- | ---------------------------------------------- |
| 满二叉树   | 所有层节点数达到最大值                            | 叶子全在最底层，节点总数 = 2^h -1（h 为高度）  |
| 完全二叉树 | 除最后一层外，其他层满；最后一层节点靠左          | 满二叉树是完全二叉树的特例，堆的底层结构       |
| 核心性质   | 任意二叉树中，**叶子节点数 = 度为 2 的节点数 +1** | 期末填空必考，比如度 2 节点有 5 个 → 叶子 6 个 |
...
//二叉树节点结构体：数据域+左右孩子的指针域
typedef struct BiTNode{
    int data;	//数据域：存节点值
    struct BiTNode *lchild; //左孩子指针--类比链表next
    struct BiTNode *rchild; //右孩子指针--类比链表next
} BiTNode,*BiTree;	//BiTree等价于BiTNode* 是根指针
```

- 二叉树和链表唯一的区别：链表只有1个`next`，二叉树有2个指针`lchild`/`rchild`
- `BiTree T`是根指针，`T=NULL`表示空树。
...
1. 关于`typedef`
   ...
   `typedef` 就像 “给类型起外号”，不创造新类型，只简化书写：

   - 基本类型：`typedef 原类型 别名;`
   - 指针类型：`typedef 原类型* 别名;`
   - 结构体：`typedef struct 结构体名 { ... } 结构体别名, 指针别名;`

   这样定义了之后，可以直接用别名定义新变量，比如说:
   ...
   - 指针类型：
     ...
     // 给int*起别名INT_PTR
     typedef int* INT_PTR;
     // 等价写法：INT_PTR p = &a; → int* p = &a;
     int a = 10;
     INT_PTR p = &a;
     printf("%d", *p); // 输出10
     ```

     ⚠️ 易错点：`typedef int* INT_PTR;` 中，`INT_PTR` 整体是 “int 指针类型”，不是 “INT_PTR p” 等价于 “int (*p)”（后者是函数指针）。
   ...
   - 结构体类型
     ...
     ```c
     // 定义结构体struct BiTNode，同时给它起别名BiTNode，给struct BiTNode*起别名BiTree
     typedef struct BiTNode {
         int data;
     ...
     | 不用 typedef           | 用 typedef（简洁） |
     | ---------------------- | ------------------ |
     | `struct BiTNode node;` | `BiTNode node;`    |
     | `struct BiTNode *T;`   | `BiTree T;`        |

     特别说明一下：

     | 特性     | typedef                 | #define（宏定义）                                  |
     | -------- | ----------------------- | -------------------------------------------------- |
     | 本质     | 类型别名                | 文本替换                                           |
     | 作用阶段 | 编译阶段                | 预处理阶段                                         |
     | 类型检查 | 有（编译时检查类型）    | 无（直接替换，不检查）                             |
     | 例子     | `typedef int* INT_PTR;` | `#define INT_PTR int*`                             |
     | 易错点   | -                       | `INT_PTR a,b;` → `int* a,b;`（b 是 int，不是指针） |
...
2. 关于这个函数的调用方式：
   ...
   // 2. 递归创建二叉树函数（你之前问的这个函数）
   void CreateBiTree(BiTree *T) {
       int val;
       scanf("%d", &val);
       if (val == -1) { // -1表示空节点，递归终止
           *T = NULL;
           return;
       }
       // 分配当前节点内存
   ...
   // 4. 主函数（核心：调用CreateBiTree）
   int main() {
       // 步骤1：定义根指针（BiTree本质是BiTNode*）
       BiTree T; 
       // 步骤2：调用创建函数，必须传「根指针的地址」（&T）
       printf("请输入二叉树节点值（-1表示空节点，按先序顺序输入）：\n");
       CreateBiTree(&T); 
       // 步骤3：遍历验证创建结果
       printf("先序遍历结果：");
       PreOrder(T);
...
3. 关于递归和函数的整体设计
   ...
   - `BiTNode` = 「节点本身」（比如一个具体的二叉树节点）；
   - `BiTree` = 「指向节点的指针」（用来找节点，是访问二叉树的 “抓手”）；
   - 你可以把`BiTree`理解成 “二叉树的根指针”，就像链表的`head`指针一样，拿到它就能操作整棵树。

   1. 首先我们解释一个问题，递归是什么？

      递归其实就是把一个”复杂大问题“拆解成”和原问题一模一样的小问题“。我们在判断一个问题能不能用递归的方法解决的时候，主要就是看是不是满足下面的三个条件:

      - 1.问题能拆成”同类型的小问题”

        比如：

        - 创建二叉树：我们可以把创建整棵树拆成 创建根节点+创建左子树+创建右子树（左/右子树的创建和整棵树的逻辑完全一样）

        - 求二叉树高度：整棵树高度=max(左子树高度，右子树告诉)+1

          （左，右子树高度的求法和整棵树逻辑完全一样）

      - 2.有明确的“终止条件”

        递归必须有终点，否则无限调用会让程序崩溃。

      - 3.小问题的结果能合并成大问题的结果

        比如二叉树的先序遍历：左子树遍历结果+右子树遍历结果，合并成整棵树遍历结果。
   ...
   2. 接下来，递归函数怎么设计呢？
      ...
      1. 步骤：
         ...
         **第一步：定义递归函数（明确 “做什么”）**
         ...
         - 确定函数参数：要操作的数据（比如二叉树的根指针）、要传递的变量（比如遍历的标记）；

         - 确定返回值：需要返回结果就写（比如求高度返回 int），不需要就写 void（比如创建二叉树、遍历）→ 例子（创建二叉树）：
         ...
         **第二步：写终止条件（先写！）**
         ...
         - 终止时要做的事：比如空节点设为 NULL、返回基础值（如阶乘返回 1）
         ...
         **第三步：拆解问题 + 递归调用**
         ...
         - 先处理 “当前层” 的逻辑（比如创建当前节点、访问当前节点）；

         - 再递归调用函数处理 “小问题”（比如创建左 / 右子树、遍历左 / 右子树）；
           ...
           → 例子（创建二叉树）
             ...
             *T = (BiTree)malloc(sizeof(BiTNode));
             (*T)->data = val;
             // 2. 递归处理小问题：创建左/右子树
             CreateBiTree(&((*T)->lchild)); // 小问题1：创建左子树
             CreateBiTree(&((*T)->rchild)); // 小问题2：创建右子树
             ```
      ...
      2. 再看两个例子：
         ...
         1. 求n的阶乘：
            ...
            // 主函数调用：factorial(5) → 5×4×3×2×1=120
            ```
   
         2. 求$\sum\limits_{n=1}^n$:
      ...
      3. 核心易错点，一定要注意：
         ...
         1. **忘记终止条件**：递归无限调用 → 栈溢出（程序崩溃）；
         2. **参数传递错误**：比如创建二叉树时传`T`（一级指针）而不是`&T`（二级指针）→ 改不了主函数的指针；
         3. **递归层次太深**：比如求 1+2+…+10000 的和 → 栈溢出（这种情况用循环）；
         4. **混淆 “当前层” 和 “子问题”**：比如遍历二叉树时，先递归再访问节点（顺序错了）→ 遍历结果不对。
   
   3. 跑一遍程序：
   
      用测试输入 `1 2 -1 -1 3 -1 -1`（对应树：1 是根，左孩子 2，右孩子 3），一步步看递归执行过程：
   
      1. **第一层递归（创建根节点 1）**：
         - 读入`1`，创建根节点，赋值`data=1`；
         - 要给根节点挂左孩子 → 调用`CreateBiTree(&根节点->lchild)`（进入第二层递归）。
      2. **第二层递归（创建左孩子 2）**：
         - 读入`2`，创建左孩子节点，赋值`data=2`；
         - 要给 2 挂左孩子 → 调用`CreateBiTree(&2->lchild)`（进入第三层递归）。
      3. **第三层递归（2 的左孩子是空）**：
         - 读入`-1`，触发终止条件 → 把`2->lchild`设为`NULL`，返回第二层。
         - 回到第二层，继续给 2 挂右孩子 → 调用`CreateBiTree(&2->rchild)`（又一层递归），读入`-1`，设为`NULL`，返回第二层。
         - 第二层递归完成，返回第一层。
      4. **回到第一层（创建右孩子 3）**：
         - 调用`CreateBiTree(&根节点->rchild)`（进入新的递归层）；
         - 读入`3`，创建右孩子节点，赋值`data=3`；
         - 递归创建 3 的左 / 右孩子，都读入`-1`，设为`NULL`；
         - 递归全部完成，整棵树创建好了。

### 2.综合应用--三种遍历、求高度、统计叶子节点个数、查找指定值的节点
...
int GetTreeHeight(BiTree T) {
    if (!T) return 0; // 空树高度为0（终止条件）
    int lh = GetTreeHeight(T->lchild); // 左子树高度
    int rh = GetTreeHeight(T->rchild); // 右子树高度
    return (lh > rh ? lh : rh) + 1;
}

// ====================== 6. 数值计算：统计叶子节点个数 ======================
// 递归思路：叶子节点 = 左子树叶子数 + 右子树叶子数；空节点0，无左右孩子1
